...
from wdapp.forms import UserForm, CompanyForm, UserFormForEdit, CargoForm,CargoForm, BusinessForm, BusinessOrderForm, StopForm,DriverExpenseForm,TripForm, DriverForm
from django.contrib.auth import authenticate, login
from django.contrib.auth.models import User
from wdapp.models import Cargo, OrderStatus, Driver,DriverExpense,DriverExpense,Trip,Stop,Business,BusinessOrder
...
@login_required(login_url='/business/sign-in/')
def business_add_cargo(request):
    form = CargoForm()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            cargo.business = request.user.business
            cargo.save()
            return redirect(business_cargo)

    return render(request, 'business/add_cargo.html', {
        "form": form
    })
@login_required(login_url='/business/sign-in/')
def business_edit_cargo(request, cargo_id):
    ...
    return render(request, 'business/edit_cargo.html', {
        "form": form
    })
def business_edit_order(request, cargo_id):
    form = BusinessOrderForm(instance = BusinessOrder.objects.get(id = business_id))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect(business_current_orders)
...
def business_sign_up(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and business_form.is_valid():
            new_user = Business.objects.create_user(**user_form.cleaned_data)
